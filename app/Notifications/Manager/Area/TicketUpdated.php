...
class TicketUpdated extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ->subject('Upravená požiadavka vo Vašej učebni - ' . $this->ticketObj->area->name . ' - PC' . $this->ticketObj->pc )
                    ->greeting('Zdravím ' . $notifiable->name)
                    ->line('V učebni ' . $this->ticketObj->area->name . ', ktorú spravujete bola upravená požiadavka.')
